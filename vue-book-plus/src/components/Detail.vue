...
export default {
    ...
    watch:{
      $route(){ // 只要路径变化 重新获取数据 ////吼吼吼
        this.getData();
      }
    },
    created(){ // 页面一加载 需要根据id 发送请求  ////根据发过来的id,获取对应的书
        this.getData();
    },
    methods: {
        async update(){ // 点击修改图书信息
          await updateBook(this.bid,this.book);     ////传两个参数，不接收返回数据
          this.$router.push('/list'); // 修改完成后跳转页面
        },
        async getData(){ // 通过id找到某一项后 赋给book
            this.book = await findOneBook(this.bid);
            // 如果是空对象 需要跳转回列表页 ////Object.keys,获取所有的key,没找到，返回列表页
            Object.keys(this.book).length>0?void 0:this.$router.push('/list');
        }
    },
    computed: {
        bid(){
            return this.$route.params.bid;//将路径参数的id映射到bid上 ////获取router上url params的方法
        }
    },
    components: {MHeader}
}
